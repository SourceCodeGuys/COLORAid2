...
def register():
    ...
    if len(password) < 6:
        return render_template('index.html', message="Password must be at least 6 characters long.")
...
def forgot_password():
    ...
    try:
        ...
        return render_template('index.html', message="Failed to send password reset email. Try again.")
...
def settings():
    ...
    if request.method == 'POST':
        ...
        lastname = request.form.get('lastname')
        username = request.form.get('username')
        email = request.form.get('email')  # Assuming client-side email update
        ...
        try:
            ...
            session['username'] = username  # Update session data
            return redirect(url_for('profile'))
        except Exception as e:
...
def test_results():
    user_id = session['user']['localId']
    user_data = db.collection('users').document(user_id).get().to_dict()  # Fetch user data
    results_ref = db.collection('test_results').where('user_id', '==', user_id).order_by('timestamp', direction=firestore.Query.DESCENDING).get()
    results = []
    ...
    return render_template('test_results.html', results=results, user_data=user_data)
...
@login_required
def save_results():
    user_id = session['user']['localId']  # Assuming the user ID is stored in session
    data = request.get_json()
    
    try:
        # Add the test result to Firestore, including detailed data
        db.collection('test_results').add({
            'user_id': user_id,
            'total_score': data.get('total_score'),
            'classification': data.get('classification'),
            'classification_scores': data.get('classification_scores'),
            'labels': data.get('labels'),  # Ensure labels are saved
            'results': data.get('results'),  # Ensure detailed results are saved
